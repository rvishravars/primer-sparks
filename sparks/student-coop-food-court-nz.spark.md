# Student Cooperative Food Court in New Zealand

# Spark Narrative

**The Observation:** Students at New Zealand universities often juggle tight budgets, busy study schedules, and limited facilities. On-campus options can make it difficult to access simple, healthy meals at a predictable, low price.

**The Shift:** Turn students from "customers" to "co-owners" of their food infrastructure. This spark proposes a **student-run cooperative food court** owned and governed by the student body. Instead of external franchises, it runs with a small paid core staff and student members who contribute time and fees. Surplus is reinvested in lower prices and better ingredients. Meals are **rationed for members** to ensure food is used for basic needs rather than commercial resale or hording.

The key question: *Can a student cooperative food court in New Zealand sustainably provide affordable, healthy food while remaining financially viable over multiple academic years?*

# Hypothesis Formalization

**Hypothesis Statement**
> "On a New Zealand university campus, a student-run cooperative food court can reduce the average cost of a standard student meal by **25–35%** compared to existing commercial vendors, while maintaining or improving self-reported meal quality and nutritional satisfaction, and covering operating costs over at least **two consecutive academic years**."

**Null Hypothesis**
> "A student-run cooperative food court does **not** achieve a ≥25% reduction in average meal cost and/or cannot sustain break-even operations over two academic years without external subsidies."

```mermaid
graph TD
    A["Student Membership & Fees"] --> B["Democratic Governance"]
    B --> C{Decision Making}
    C -->|Menu & Sourcing| D["Bulk Purchasing"]
    C -->|Operations| E["Cooking & Service (Blended Labor)"]
    D --> E
    E --> F["Meal Sales & Rationing"]
    F --> G{Operating Surplus?}
    G -->|Yes| H["Lower Prices & Higher Quality"]
    G -->|No| I["Operational Review"]
    H --> A
```

# Testing & Results

## Methodology
The hypothesis is tested via a **pilot program** at a single high-traffic university campus in New Zealand. The pilot involves launching a cooperative food court with a limited menu of staple, low-cost healthy meals. Key design elements include a membership model, low annual fees, fair-use rationing, and a blended labor model (paid core staff plus student member shifts). We prioritize local suppliers and bulk purchasing cooperatives to reduce unit costs.

Measurement is conducted through daily sales volume tracking, monthly P&L reporting (including rent, utilities, and wages), and termly student surveys. We evaluate affordability by comparing the coop's meal prices against comparable campus vendors and investigate financial stability based on break-even performance without external subsidies over two academic years.

## Outcomes
Observed results focus on daily meal throughput and monthly net operating income. We monitor membership renewal rates and adherence to fair-use rationing as proxies for cooperative health. Revision notes from early trials focus on refining the membership and labor models to balance student participation with operational efficiency.

---